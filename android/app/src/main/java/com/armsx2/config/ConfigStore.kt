...
object ConfigStore {
    ...
    private const val KEY_INFORMATIVE_OSD_MIGRATED = "config.migrated.informativeOsd"
    /**
    ...
    /**
     * One-time: turn on the informative OSD rows for installs that predate them.
     *
     * The stats block (R3000A + GTE, GPU primitives/fill, SPU/MDEC/CD-ROM/DMA, frame times,
     * resolution, renderer) shipped with every row defaulted OFF, so the overlay showed only
     * "FPS 58.6 / 99%" and looked like the feature had never landed. Changing the Kotlin
     * defaults alone does not help anyone who already has settings saved — a persisted `false`
     * beats a new default every time. Hence this.
     *
     * Deliberately does NOT force `osdShowMessages` or the OSD mode: those are already on, and
     * a migration that overrides a choice the user actually made would be worse than the bug.
     */
    fun migrateInformativeOsd(context: android.content.Context) {
        if (MainActivityRuntime.prefs.getBoolean(KEY_INFORMATIVE_OSD_MIGRATED, false)) return
        MainActivityRuntime.prefs.edit().putBoolean(KEY_INFORMATIVE_OSD_MIGRATED, true).apply()
        // Fresh installs already get these from the Settings defaults.
        if (MainActivityRuntime.prefs.getString(KEY_GLOBAL, null) == null) return
        val g = loadGlobal()
        saveGlobal(
            g.copy(
