...
data class Settings(
     ...
     *  emulation thread, which also presents, so there is no order to choose — only which
     *  cores it may use. 7 stays accepted as an alias for 1 so an inherited value still means
     *  "performance cores". Implemented in frontend/perf_hint.c, which detects the performance
     *  cluster from each core's cpuinfo_max_freq rather than assuming core indices.
     *
     *  Pushed to native via NativeApp.setAffinityMode before runVMThread, and again live from
     *  the Graphics tab / pause menu rows. Per-game because a heavier title can want the pin
     *  while a light one does better left alone. Off is still the recommended default —
     *  Android's EAS scheduler usually beats hand-pinning. */
    ...
    // 50, down from 65: at 65 the stats block ran under the on-screen face buttons on a
    // landscape phone, so the numbers and the controls fought for the same pixels.
    val osdScale: Int = 50,
    ...
    // Host DEVICE usage (cpu %, ram, gpu busy) as opposed to the emulated machine's counters.
    // Deliberately has NO NativeApp.osdShowHostUsage() push: GameOsd reads the setting straight
    // from this object, and the neighbouring NativeApp.osdShow* calls are inert PS2-era stubs.
    val osdShowHostUsage: Boolean = true,
    ...
    fun applyTo() {
        ...
        // EE/FPU clamping (recompiler accuracy). The mode unpacks to the PCSX2
        // bit flags below. Needs a recompiler reset (commitSettings / game
        ...
        // ★ FRAME PACING IS NOT WRITTEN HERE ANY MORE.
        //
        // This block used to persist EmuCore/GS FrameLimitEnable and Framerate/NominalScalar and
        // then poke speedhackLimitermode() / setNominalSpeed() / setFpsCap() — every one of them a
        // PCSX2 entry point with an empty body in this port. The frame-limit switch, the emulation
        // speed percentage and the display FPS cap all moved, all persisted, and none of them ever
        // reached the emulation thread; that is exactly the "display fps cap doesn't work" report.
        //
        // They now live in settings.toml's [runtime] table (frame_limit / speed_percent /
        // fps_limit / fast_forward_speed) and reach ArmsxSession::targetFrameRate() through
        // com.armsx2.core.Ps1Pacing → NativeApp.setSpeedLimits(). MainActivityRuntime re-pushes
        // that on every settings apply. Do not re-add the stub calls alongside it.
    ...
    fun readFromIni(ini: Map<String, String>): Settings {
        ...
        return this.copy(
            ...
            osdShowHostUsage = boolAt("EmuCore/GS/OsdShowHostUsage") ?: this.osdShowHostUsage,
    ...
    private fun writeGsToNative() {
        ...
        //
        // ★ THE CHAIN ITSELF IS SELECTED HERE, NOT BY THE TWO put() CALLS ABOVE. Those write
        // EmuCore/GS keys through NativeApp.setSetting, which is an EMPTY STUB in this port —
        // the whole EmuCore INI path is dead here. They are kept only because they are what
        // the settings export and the fresh-install INI recovery read back (see fromNativeIni).
        // The live path is this native setter, and dropping it is how the toggle would render,
        // move, persist and reach nothing at all.
        if (emitSink == null) {
            NativeApp.setShaderChain(shaderChainEnabled, shaderChainPreset)
        ...
        put("EmuCore/GS", "OsdShowHostUsage", "bool", osdShowHostUsage.toString())
    ...
    fun toJson(): JSONObject = JSONObject().apply {
        ...
        put("osdShowHostUsage", osdShowHostUsage)
    ...
    companion object {
         ...
         *    - GPU palette conversion off               — drop optional GPU work
        ...
        fun fromJson(json: JSONObject): Settings {
            ...
            return Settings(
                ...
                // 65 was the PREVIOUS default and it overlapped the on-screen face buttons.
                // A stored value of exactly 65 therefore means "never customised", so migrate it
                // to the new default rather than leaving every existing install on the old size —
                // changing a default does nothing for anyone who already has the value persisted.
                // Safe to delete once no pre-release install carries 65.
                osdScale = json.optInt("osdScale", def.osdScale).let { if (it == 65) def.osdScale else it },
                ...
                osdShowHostUsage = json.optBoolean("osdShowHostUsage", def.osdShowHostUsage),
        ...
        fun diff(base: Settings, current: Settings): JSONObject {
            ...
            if (current.osdShowHostUsage != base.osdShowHostUsage) j.put("osdShowHostUsage", current.osdShowHostUsage)
        ...
        fun merge(base: Settings, overrides: JSONObject): Settings = Settings(
            ...
            osdShowHostUsage = if (overrides.has("osdShowHostUsage")) overrides.getBoolean("osdShowHostUsage") else base.osdShowHostUsage,
