 ...
 *
 *  1. `API_GetGameList` (i=12 PlayStation, f=1 with-achievements, h=1 with-hashes) — one request
 *     giving every set's size keyed by MD5. Cached on disk; set sizes barely ever change.
 ...
 * boot executable, no boot required) and matched against the catalogue by hash — the only reliable
 * key, since RA carries no disc serials and title matching would mismatch regional variants.
 *
 ...
 */
object RaLibrary {
    // ★ Was 21 (RC_CONSOLE_PLAYSTATION_2), lifted straight from ARMSX2. On a PS1 emulator that
    // asked RetroAchievements for the PlayStation *2* catalogue, so no library game's MD5 could
    // ever match and the whole sync silently produced nothing. 12 is the id the core itself uses
    // (frontend/achievements.cpp: kConsoleId = RC_CONSOLE_PLAYSTATION).
    private const val CONSOLE_ID = 12               // rc_consoles.h: RC_CONSOLE_PLAYSTATION
    private const val API = "https://retroachievements.org/API"
    // Renamed with the console id: a cache file written against the PS2 catalogue must not be
    // read back as if it were the PlayStation one.
    private const val CATALOG_FILE = "ra_psx_catalog.json"
    ...
    private fun loadOrFetchCatalog(context: Context, userName: String, key: String): Catalog? {
        ...
        val body = get("$API/API_GetGameList.php?i=$CONSOLE_ID&f=1&h=1" +
    ...
    private fun fetchUserProgress(userName: String, key: String): Map<Int, Pair<Int, Int>>? {
        ...
        while (true) {
            ...
            for (i in 0 until results.length()) {
                ...
                if (r.optInt("ConsoleID", r.optInt("consoleId", -1)) != CONSOLE_ID) continue
