...
object InGameOverlay {
    ...
    /** Mirror of `[runtime] frame_limit` in settings.toml, for the pause menu's switch and the
     *  touch overlay. Owned by [com.armsx2.core.Ps1Pacing]; this is a Compose-observable copy,
     *  NOT a second store. It used to mirror the PS2 `Settings.frameLimitEnable`, whose native
     *  writes were empty PCSX2 stubs. */
    ...
    fun saveSettings(updated: Settings) {
        ...
        if (MainActivityRuntime.nativeReady.value) {
            runCatching {
                // The frame limit is NOT taken from `updated` here any more. It lives in
                // settings.toml, is written by Ps1Pacing.setFrameLimit() from the row that owns
                // it, and the two native calls this used to make — setSetting("EmuCore/GS",
                // "FrameLimitEnable") and speedhackLimitermode() — are empty PCSX2 stubs. What is
                // still needed is a re-push, because a settings apply must not drop the pacing.
                MainActivityRuntime.instance?.applicationContext?.let { ctx ->
                    com.armsx2.core.Ps1Pacing.push(ctx)
                }
    ...
    fun open() {
        ...
        // From settings.toml, not the PS2 Settings object: this is what the core is actually
        // pacing to, and it is what the switch in the pause menu writes back.
        MainActivityRuntime.instance?.applicationContext?.let { ctx ->
            com.armsx2.core.Ps1Pacing.invalidate()
            frameLimitOn.value = com.armsx2.core.Ps1Pacing.frameLimit(ctx)
        }
