...
object TouchControls {
    ...
    /** D-pad arm gap, as a fraction of the pad. 7% separates the four directions enough to
     *  read (and press) as distinct keys instead of one solid cross. */
    const val DPAD_SPACING_DEFAULT = 0.07f

    val dpadSpacing = mutableFloatStateOf(DPAD_SPACING_DEFAULT)
    ...
    // ---- Pressure sensitivity: INERT on the PS1 port ----------------------
    //
    // DualShock2 reported an 8-bit force per button; the PS1 DualShock reports one BIT.
    // There is no soft press to send, so the whole modifier is a no-op here (see
    // [pressureRangeFor], which now always returns the full-press sentinel).
    //
    // The state below is NOT deleted: MainActivityRuntime.kt (not owned by this port)
    // writes pressureModifierHeld from the PRESSURE_MOD hotkey's hold handler. Keeping
    // the property means that file still compiles while the value stops mattering.

    /** Held-state for the old DS2 pressure modifier. Still written by
     *  MainActivityRuntime's PRESSURE_MOD hold handler, but no longer read by anything
     *  that reaches the core — PS1 buttons are digital. */
    ...
    /**
     * Range to send for [keycode]. On the PS1 port this is ALWAYS 0 — the "full press"
     * sentinel — because every PlayStation pad button is a single bit.
     *
     * The PS2 build consulted a PRESSURE_KEYCODES set (d-pad, face buttons, L1/L2/R1/R2)
     * and scaled the range by [pressurePercent]. That table is gone: on PS1 there is no
     * partial press to express, and emitting a reduced range would just be a differently
     * spelled full press once the value is bridged to psx_pad_button().
     */
    @Suppress("UNUSED_PARAMETER")
    fun pressureRangeFor(keycode: Int): Int = 0
    ...
    val macroAssignableTargets: List<MacroTarget> = listOf(
        ...
        // 200 = PAD_ANALOG (the DualShock analog/mode button — PS1 DualShock has one too).
        MacroTarget(200, "Analog"),
        // PS1 port: the "Pressure" macro target was removed from this picker (PS1 buttons
        // are digital). MACRO_CODE_PRESSURE itself stays defined so already-saved macros
        // that contain it keep deserialising — fire() just filters it out harmlessly.
    ...
    private fun load() {
        ...
        dpadSpacing.floatValue = MainActivityRuntime.prefs.getFloat(KEY_DPAD_SPACING, DPAD_SPACING_DEFAULT).coerceIn(0.0f, 0.35f)
