...
private fun ControllerOptions(
    ...
    GlassPanel(modifier) {
        Column(verticalArrangement = Arrangement.spacedBy(7.dp)) {
            ...
            // PS1 port: the Multitap switch was removed here. The ARMSX core hard-codes pad
            // index 0, so there is no 3rd..8th pad destination to enable (see PadRouter).
            SettingSwitchRow(
...
private fun ControllerBindings(state: ControllerManagerUiState, viewModel: ControllerManagerViewModel, modifier: Modifier) {
    Column(modifier) {
        ...
        if (state.section == ControllerSection.Buttons) {
            ...
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                // PS1 port: iterate the supported subset, NOT SysHotkey.entries — the enum
                // still carries every PS2 hotkey so MainActivityRuntime's exhaustive `when`
                // blocks keep compiling. See ControllerMappings.ps1Hotkeys for what the
                // ARMSX core can actually serve and why the rest were dropped.
                ControllerMappings.ps1Hotkeys.forEach { hotkey ->
                    BindingRow(
