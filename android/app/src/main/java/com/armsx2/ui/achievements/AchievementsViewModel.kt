...
class AchievementsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // SOFTCORE ONLY. ARMSX has no hardcore path: the emulator needs a lot more testing before
    // an unlock earned on it may claim hardcore credit on someone's RetroAchievements account.
    // This is inert, the switch that called it is gone from AchievementAccount, and the native
    // NativeApp.setHardcoreMode is a logged no-op — so the confirm dialog below can never open.
    ...
    private fun parse(json: String): AchievementsUiState {
        ...
        return AchievementsUiState(
            ...
            // Always false — ARMSX RetroAchievements support is softcore only. Left in the
            // model (rather than deleted) because the score chips and DiscordPresence still
            // read it; it simply never becomes true.
            hardcore = false,
