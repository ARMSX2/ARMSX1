 ...
 *
 *  Default ON. Keep this default and BootSplashActivity's `getBoolean` default
 *  identical — the splash reads the raw preference before this object loads, so
 *  disagreeing defaults would play the intro on first launch while the toggle
 *  showed it off. */
...
object LibraryBackgroundColorPreferences {
    ...
    /** The default wave color (ARMSX1 orange), shown in the picker when unset and what actually
     *  renders until the user picks something else. */
    const val DefaultDisplayColor: Int = 0xFFF35112.toInt()
    ...
    val PRESETS: List<Int> = listOf(
        0xFFF35112.toInt(), // ARMSX1 orange (default)
    ...
    private fun apply(argb: Int) {
        // 0 = "not customized", which for ARMSX1 means the default orange rather than
        // XmbGlView's own built-in blue gradient — so the shipped look is the orange.
        val effective = if (argb == 0) DefaultDisplayColor else argb
        val r = ((effective shr 16) and 0xFF) / 255f
        val g = ((effective shr 8) and 0xFF) / 255f
    ...
    // Seed the gradient with the default orange so a GL frame drawn before load()
    // (cold start race) never shows XmbGlView's built-in blue.
    init {
        apply(0)
    }
}
