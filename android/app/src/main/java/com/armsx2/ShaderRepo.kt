...
object ShaderRepo {
    ...
    /** The CRT pack shipped inside the APK, under `assets/shaders/`. */
    private const val BUNDLED_ASSET_DIR = "shaders"

    /** Bump when the bundled pack's CONTENT changes, to re-install it over the old copy.
     *  Not the app version: re-copying on every release would silently revert a user's
     *  hand-edits to a bundled preset every time they updated. */
    private const val BUNDLED_VERSION = 1

    private const val BUNDLED_STAMP = ".armsx-bundled"

    /**
     * Install the bundled CRT pack into the shaders root, so shaders work out of the box
     * without a 51 MB download. Idempotent and cheap on the common path — one stat of the
     * stamp file — so it is safe to call before every scan, which is exactly what it is for:
     * the pack appears the first time the user opens the picker, and costs nothing at boot
     * for the majority who never do.
     *
     * A user who DELETES the bundled pack keeps it deleted: the stamp survives the delete,
     * so only a [BUNDLED_VERSION] bump brings it back. That is the intended escape hatch —
     * an install-on-sight loop that resurrected a pack the user removed would be
     * unfixable from inside the app.
     *
     * Blocking IO. Returns true if anything was written.
     */
    fun installBundled(context: Context): Boolean {
        val root = shadersRoot(context)
        val stamp = File(root, BUNDLED_STAMP)
        if (runCatching { stamp.readText().trim().toInt() }.getOrNull() == BUNDLED_VERSION) {
            return false
        }

        val assets = context.assets
        val packs = runCatching { assets.list(BUNDLED_ASSET_DIR) }.getOrNull() ?: emptyArray()
        var wrote = false
        for (pack in packs) {
            if (copyAssetTree(context, "$BUNDLED_ASSET_DIR/$pack", File(root, pack))) wrote = true
        }

        // Written even when nothing was copied (a build with no bundled assets), so a
        // shader-less build does not re-walk the asset tree on every single scan.
        runCatching { stamp.writeText(BUNDLED_VERSION.toString()) }
        return wrote
    }

    /** Recursively copy an asset directory. `assets.list` returns a non-empty array for a
     *  directory and an empty one for a file, which is the only way to tell them apart
     *  through AssetManager. */
    private fun copyAssetTree(context: Context, assetPath: String, target: File): Boolean {
        ...
        for (child in children) {
            if (copyAssetTree(context, "$assetPath/$child", File(target, child))) wrote = true
        }
        return wrote
    }
