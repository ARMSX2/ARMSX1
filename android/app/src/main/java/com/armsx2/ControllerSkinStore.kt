...
object ControllerSkinStore {
    ...
    /** The out-of-box skin. A user who has never picked one gets ARMSX1 (bagas's pack) rather
     *  than the bundled pad_* drawables, which are the ARMSX2 look and are still selectable as
     *  "ARMSX2". Applied at every point a stored preference reads back null, so the default can
     *  never disagree with itself between the resolver and the query helper below. */
    const val DEFAULT_SKIN_ID = "builtin:armsx1"

    val activeSkinId = mutableStateOf<String?>(DEFAULT_SKIN_ID)
    ...
    fun applyForSerial(ctx: Context, serial: String?) {
        ...
        val raw = if (eff != null)
            MainActivityRuntime.prefs.getString(KEY_ACTIVE_GAME_PREFIX + eff, null)
                ?: MainActivityRuntime.prefs.getString(KEY_ACTIVE, null) ?: DEFAULT_SKIN_ID
        else MainActivityRuntime.prefs.getString(KEY_ACTIVE, null) ?: DEFAULT_SKIN_ID
    ...
    val BUILTIN: List<BuiltinSkin> = listOf(
        // Order IS the presentation order. ARMSX1 (by bagas) leads as the app's own look;
        // ARMSX2 is the PS2 sibling's skin kept as a familiar second; the two NetherSX2 sets
        // stay where they were so anyone using them keeps their selection.
        BuiltinSkin("builtin:armsx1", "ARMSX1", "controller_skins/armsx1"),
        BuiltinSkin("builtin:armsx2", "ARMSX2", "controller_skins/armsx2"),
