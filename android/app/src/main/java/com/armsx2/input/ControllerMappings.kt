...
object ControllerMappings {
    ...
    /**
     * The PS1 (DualShock) button set.
     *
     * Worth stating plainly, because it is the thing most likely to be "fixed" by mistake:
     * this list did NOT need to shrink for the PS1 port. The DualShock and the DualShock2
     * have the SAME buttons — d-pad, Cross/Circle/Square/Triangle, L1/L2/R1/R2, L3/R3,
     * Start, Select, two analog sticks and the Analog/mode toggle. The PS2 pad's extra
     * capability was pressure sensitivity (an 8-bit force per button), not extra buttons,
     * and that is handled separately: see TouchControls.pressureRangeFor, now hard-wired
     * to full-press.
     *
     * So every entry below maps 1:1 onto a PSXI_SW_SDA_* bit in frontend/main.cpp
     * (buttonForKey / buttonForController). The eight "(send)" stick-direction targets at
     * the end are app-side conveniences — they let a button drive a stick axis — and land
     * on psx_pad_analog_change()'s PSXI_AX_SDA_* axes.
     *
     * What the PS1 port DOES remove lives elsewhere in this file and in the UI: multitap
     * (PS2-only, and the core drives pad 0 only), pressure sensitivity, USB devices
     * (GunCon/keyboard/mic), and the unsupported system hotkeys — see [ps1Hotkeys].
     */
    val actions = listOf(
        ...
        // The DualShock Analog/mode button. Target 200 -> PAD_ANALOG. UNBOUND by default —
        // there's no standard Android keycode for it, so the user binds a button. On PS1
        // this matters MORE than it did on PS2: many titles boot the pad in digital mode
        // and only expose the sticks after the analog button is pressed. The ARMSX core
        // has the matching PSXI_SW_SDA_ANALOG bit (frontend/main.cpp, keyboard '2').
        Action("analog", "Analog (toggle)", 200, KeyEvent.KEYCODE_UNKNOWN),
        // ---- PS1 analog-stick DIRECTIONS as bindable targets (AetherSX2-style) ----
    ...
    // Multitap — REAL on this port now (psx/input/multitap.c). One PlayStation Multitap in
    // controller port 1 carries four pads, so PadRouter claims up to four slots and the core
    // routes each to its own sub-controller.
    //
    // The state lives in settings.toml ([input] multitap), NOT in SharedPreferences: the core
    // reads that file when it boots, and a value kept only in prefs would be a UI switch the
    // emulator never saw — which is exactly what these two used to be (permanently-false
    // stubs, with an in-game switch wired to them).
    //
    // Both functions need a Context and are called from places that have none in scope, so
    // they go through the Activity's application context and fall back to the compiled-in
    // default when no Activity is up (during which nothing is running to route input to
    // anyway).
    private fun ps1Context(): android.content.Context? =
        MainActivityRuntime.instance?.applicationContext

    fun multitapEnabled(): Boolean {
        val ctx = ps1Context() ?: return false
        return runCatching { com.armsx2.config.Ps1SettingsStore.active(ctx).multitap }
            .getOrDefault(false)
    }

    /** Persist the tap state, push it into the running core, and re-cap slot routing. All
     *  three, because any one on its own is a half-wired setting: the file alone reverts on
     *  the next frame, the native alone reverts on the next launch, and without the router
     *  players 2-4 can never claim a slot to be routed. */
    fun setMultitapEnabled(on: Boolean) {
        PadRouter.setMultitapEnabled(on)

        val ctx = ps1Context() ?: return

        runCatching {
            val store = com.armsx2.config.Ps1SettingsStore
            store.save(ctx, store.load(ctx).copy(multitap = on))
        }

        runCatching { kr.co.iefriends.pcsx2.NativeApp.setMultitapEnabled(on) }
    }
    ...
    /** A PS1 pad button a stick direction may map to. The numeric codes are inherited
     *  from the PS2 build's setPadButton map and are kept as-is: they are just the
     *  app-side code space, and the PS1 pad has exactly the same button complement
     *  (d-pad, 4 face, L1/L2/R1/R2, L3/R3, Start/Select, analog toggle). Whatever
     *  bridges input to the core translates these into PSXI_SW_SDA_* bits.
     *  [label] drives the picker UI. */
    ...
    enum class SysHotkey(val prefKey: String, val label: String) {
        ...
        // Hold to run the game BACKWARDS through the rewind buffer ([emulation] rewind).
        // A hold, like FAST_FORWARD: the natural gesture is "hold until you are back before
        // the mistake, let go, play on". Does nothing until Rewind is switched on in
        // Settings › Emulation — the buffer is not allocated otherwise.
        // Appended last for the same persisted-by-ordinal reason as the two above: hotkeys
        // are stored by enum index, so inserting one mid-enum re-points existing bindings.
        REWIND("pad.rewind.keycode", "Rewind (hold)"),
    }

    // ---- PS1 port: which hotkeys the ARMSX (PS1) core can actually serve ----
    //
    // The enum above is deliberately left INTACT even though most of it is dead on this
    // port. MainActivityRuntime.kt contains two exhaustive `when (hotkey)` blocks over
    // SysHotkey; deleting entries would break a file this port does not own. So the enum
    // stays complete and the UI narrows instead — [ps1Hotkeys] is the only list any
    // hotkey screen should iterate.
    //
    // Kept, because frontend/main.cpp has the corresponding machinery:
    //   MENU                 -> pause; ArmsxSession::setPaused() exists and the input
    //                           router already raises a pause request (Esc / pad chord).
    //   FAST_FORWARD_TOGGLE  -> ArmsxSession::setFastForwardEnabled() + the
    //                           deferred_fast_forward_toggle_ pump.
    //   SCREENSHOT           -> the deferred_screenshot_ pump (writes a PNG).
    //   RESET_GAME           -> ArmsxSession::reset() -> psx_soft_reset().
    //   CLOSE_GAME/QUIT_APP  -> purely app-side (finish the SDL activity), so they work
    //                           regardless of what the core exposes.
    //
    // Dropped, because the PS1 core has NO such concept at all:
    //   SAVE_STATE / LOAD_STATE / CYCLE_SLOT / SAVE_AND_EXIT — there is no save-state
    //     implementation anywhere in frontend/main.cpp. Not "unwired": absent.
    //   RES_UP/RES_DOWN— internal resolution is a settings.toml value read at startup;
    //                    there is no runtime resolution step.
    //   ACHIEVEMENTS   — no RetroAchievements client.
    //   SLOW_DOWN      — no slow-motion limiter (only the FF toggle exists).
    //   TOGGLE_OSD     — the imgui overlays went away with the FSUI cut.
    //   PRESSURE_MOD   — DualShock2 pressure sensitivity; PS1 pads are digital.
    //   TOGGLE_KEYBOARD— PS2 USB keyboard passthrough; no PS1 equivalent.
    //   GYRO_*         — app-side motion that feeds NativeApp's analog path. That path is
    //                    live now (setPadAnalog is `native` again), so this one is only
    //                    dropped by choice, not because it would no-op.
    //
    // NOTE: this list predates the in-process JNI bridge and is due a re-audit — the core
    // is no longer reachable through a single launch-argument symbol. The pad itself is
    // fully wired (setPadButton/setPadAnalog/resetKeyStatus -> psxe_host_pad_*), and so
    // are save states (saveStateToSlot/loadStateFromSlot are `native`). Before restoring
    // any entry, grep NativeApp.java and confirm the method is `native`, not a stub.
    val ps1Hotkeys: List<SysHotkey> = listOf(
        SysHotkey.MENU,
        // Real on this port since psx/texrep.c landed: flips [video] texture_dump and
        // re-pushes it, which is the same path the Video tab's switch takes. A hotkey earns
        // its place here because dumping is used WHILE playing — you walk into a room, dump
        // what it drew, and turn it back off.
        SysHotkey.TEXTURE_DUMP,
        SysHotkey.FAST_FORWARD_TOGGLE,
        // Rewind is a real core feature on this port now (psx/rewind.c) — hold to step the
        // machine back through its snapshot ring. Listed here because a hotkey is the ONLY
        // way to reach it while playing; the settings rows only size the buffer.
        SysHotkey.REWIND,
        SysHotkey.SCREENSHOT,
        SysHotkey.RESET_GAME,
        SysHotkey.CLOSE_GAME,
        SysHotkey.QUIT_APP,
    )
