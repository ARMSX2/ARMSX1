...
class EmulationSurface(context: Context) :
    ...
    fun applyOutputScale() {
        ...
        // hwScaler (if > 0) downscales the short side to a multiple of the console's NATIVE
        // vertical resolution for weak GPUs, applied to whatever base we picked above.
        //
        // 240, not the 448 this arrived with. That constant is the PlayStation 2's native height
        // and came over with the lifted UI; on a PS1 whose native frame is 240 lines it made
        // "1× native" mean 448 — nearly 2× — so the lowest setting still asked the GPU for almost
        // twice the pixels the option claimed, and the whole control was compressed into a range
        // that never reached native at all.
        val shortSide = minOf(baseW, baseH)
        val targetShortSide = if (multiplier > 0) PS1_NATIVE_HEIGHT * multiplier else shortSide
