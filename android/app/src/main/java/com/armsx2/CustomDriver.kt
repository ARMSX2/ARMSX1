...
object CustomDriver {
    ...
    /**
     * Key holding the id of a driver whose load was STARTED but never confirmed working.
     *
     * A custom GPU driver runs before anything can catch it: a bad one takes the process down
     * with a native signal, on every launch, forever. Recovering then means clearing app data or
     * editing prefs over adb, neither of which a user can do — the app is simply bricked, and it
     * is bricked by a menu item that looks as harmless as any other.
     *
     * So the selection is armed BEFORE the load and disarmed only once a frame has actually been
     * presented on it. Finding it still armed at startup means the last run died holding it.
     */
    private const val PENDING_KEY = "ui.customDriver.pending"

    /**
     * Call at startup, BEFORE any driver is applied. If the previous run died with a driver
     * armed, that driver is dropped and its id returned so the caller can tell the user why they
     * are back on the system driver. Returns null on a clean start.
     */
    fun consumeCrashedDriver(context: Context): String? {
        val prefs = MainActivityRuntime.prefs
        val pending = prefs.getString(PENDING_KEY, "").orEmpty()

        if (pending.isBlank()) return null

        // Disarm and deselect. Deselecting is the point: leaving it selected would re-arm the
        // same driver on the next launch and reproduce the crash indefinitely.
        prefs.edit()
            .remove(PENDING_KEY)
            .putString("ui.customDriverId", "")
            .apply()

        Log.w(TAG, "Custom driver '$pending' did not survive the last launch; reverting to the system driver.")

        return pending
    }

    /** Called once the renderer has actually presented a frame: the driver is proven good. */
    fun markDriverHealthy() {
        runCatching { MainActivityRuntime.prefs.edit().remove(PENDING_KEY).apply() }
    }
    ...
    fun applyToNative(context: Context, installed: InstalledDriver?) {
        ...
        // Arm BEFORE handing the driver to native. If the process dies during or after the load,
        // this is what is left behind for the next launch to find.
        runCatching {
            MainActivityRuntime.prefs.edit().putString(PENDING_KEY, installed.id).apply()
        }
