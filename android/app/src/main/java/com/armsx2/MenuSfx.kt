...
object MenuSfx {
    ...
    /** Volatile: [rebuildPool] runs on a worker at launch (see [load]); every reader is on the UI thread. */
    @Volatile private var pool: SoundPool? = null
    /** event -> loaded SoundPool sample id. */
    // Concurrent, not plain HashMaps: [load] builds the pool on a worker (off the cold-start path)
    // while play() reads these from the UI thread, and a HashMap resized under a concurrent read
    // does not merely lose an entry — it can spin.
    private val sampleIds = java.util.concurrent.ConcurrentHashMap<Event, Int>()
    ...
    private val altSampleIds = java.util.concurrent.ConcurrentHashMap<Event, Int>()
    ...
    private val lastPlayMs = java.util.concurrent.ConcurrentHashMap<Event, Long>()
    ...
    /**
     * Restore the saved state at launch.
     *
     * The prefs reads stay inline (they feed Compose state that the settings UI reads immediately),
     * but the pool build does NOT: [rebuildPool] constructs a SoundPool and issues 15 `load()` calls,
     * each of which opens a resource/file descriptor. That is real I/O on the cold-start path for
     * sounds nobody can trigger until the library is on screen and being navigated. SoundPool is
     * safe to build and load from any thread, and [play] no-ops until [pool] is set.
     */
    fun load(context: Context) {
        ...
        if (enabled.value) {
            ...
            kotlin.concurrent.thread(isDaemon = true, name = "armsx-menusfx-load") {
                runCatching { rebuildPool(app) }
                    .onFailure { Log.w(TAG, "deferred pool build failed", it) }
            }
        }
    }
